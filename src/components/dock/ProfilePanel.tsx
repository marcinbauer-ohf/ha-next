...
/**
 * The embedded page with HA's own navigation cropped away.
 *
 ...
 */
function CroppedFrame({ src, title }: { src: string; title: string }) {
  ...
  return (
    <div
      ref={boxRef}
      className="relative h-full overflow-hidden rounded-xl border border-neutral-200 bg-white"
    >
      <iframe
...
function DetailPane({ item }: { item: DockItem }) {
  ...
  return <CroppedFrame src={src} title={item.label} />;
}
