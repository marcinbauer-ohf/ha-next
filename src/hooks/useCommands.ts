...
export function useCommands(): CommandItem[] {
  ...
  return useMemo<CommandItem[]>(() => {
    ...
    const commands: CommandItem[] = [
      ...
      {
        id: 'dbg.settings-rail',
        group: 'debug',
        icon: mdiDockLeft,
        label: 'Settings list as a shell rail',
        keywords: ['settings', 'sidebar', 'rail', 'column', 'nav'],
        state: onOff(debugFlags.settingsRailEnabled),
        active: debugFlags.settingsRailEnabled,
        run: debugFlags.toggleSettingsRail,
      },
      {
